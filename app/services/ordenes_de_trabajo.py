...
class OrdenDeTrabajoService:
    ...
    async def get_detail(self, orden_id: UUID, user) -> OrdenTrabajoDetailOut:
        ...
        sup_doc = db_firestore.collection("users").document(o.supervisor_id).get()
        supervisor_nombre = sup_doc.to_dict().get("display_name", "—") if sup_doc.exists else "—"
        ...
        cliente_nombre = "—"
        if cliente_id:
            cli_doc = db_firestore.collection("users").document(cliente_id).get()
